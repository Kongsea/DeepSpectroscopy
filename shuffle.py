...
def shuffle_bin(spec_name, spec_shuffle_name, shuffle_seed, spec_length, bit_length=4):
  length = (1 + spec_length) * bit_length
...
def main():
  ...
  shuffle_seed_name = 'shuffle_seed.npy'
  shuffle_seed = np.random.permutation(spec_num)
  with open(shuffle_seed_name, 'wb') as frs:
    np.save(frs, shuffle_seed)
  
  shuffle_csv(csv_lines, 'shuffle.csv', shuffle_seed)
  
  spec_length = 57144
  spec_name = 'original.bin'
  spec_shuffle_name = 'shuffle.bin'
